...
class Modifier(db.Model, ModelMixin, ModelSerializer):
    ...
    __table_args__ = (UniqueConstraint("letter_multiplier", "word_multiplier"),)
...
class PositionedModifier(db.Model, ModelMixin, ModelSerializer):
    ...
    __table_args__ = (UniqueConstraint("row", "column", "modifier_id"),)

    row = db.Column(
        db.Integer, nullable=False, doc="The 0-indexed row number of the position."
    )
    column = db.Column(
        db.Integer, nullable=False, doc="The 0-indexed column number of the position."
    )
...
class BoardLayout(db.Model, MetadataMixin, ModelSerializer):
    ...
    name = db.Column(
        db.String(256), nullable=False, doc="A descriptive name for the board layout."
    )
    ...
    rows = db.Column(
        db.Integer, nullable=False, doc="The number of rows in the layout."
    )

    columns = db.Column(
        db.Integer, nullable=False, doc="The number of columns in the layout."
    )
