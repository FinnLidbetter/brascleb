...
class Tile(db.Model, ModelMixin, ModelSerializer):
    ...
    __tablename__ = "tile"
    __table_args__ = (UniqueConstraint("letter", "is_blank", "value"),)
    letter = db.Column(
        db.String(1, collation="NOCASE"),
        nullable=True,
        doc="The letter to display on this tile (possibly " "None).",
    )
    is_blank = db.Column(
        db.Boolean,
        nullable=False,
...
class TileCount(db.Model, ModelMixin, ModelSerializer):
    ...
    __table_args__ = (UniqueConstraint("tile_id", "count"),)
    serialize_exclude_fields = ["tile_id"]

    count = db.Column(
        db.Integer, nullable=False, doc="The number of copies of the tile."
    )
...
class PlayedTile(db.Model, ModelMixin, ModelSerializer):
    ...
    __table_args__ = (UniqueConstraint("tile_id", "row", "column"),)
    tile_id = db.Column(db.Integer, db.ForeignKey("tile.id"), nullable=False)
    tile = relationship("Tile", doc="The tile played.")
    row = db.Column(
        db.Integer, nullable=False, doc="The 0-based row index. The top row is row 0."
    )
    column = db.Column(
        db.Integer,
        nullable=False,
        doc="The 0-based column index. The left column " "is column 0.",
    )
...
class Distribution(db.Model, MetadataMixin, ModelSerializer):
    ...
    __table_args__ = (UniqueConstraint("creator_id", "name"),)
    ...
    creator = db.relationship(
        Player,
        primaryjoin=Player.id == creator_id,
        post_update=True,
