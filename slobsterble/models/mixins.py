...
class ModelSerializer:
    ...
    def serialize(self, exclusions=None, override_mask=None, sort_keys=None):
        ...
        for column in model_columns:
            if (
                column in self.base_exclude_fields
                and column not in self.serialize_include_fields
            ):
                continue
            ...
            if (
                ...
                and type(self).__name__ in exclusions
    ...
    def serialize_list(items, exclusions=None, override_mask=None, sort_keys=None):
        ...
        serialized_list = [
            item.serialize(exclusions, override_mask, sort_keys) for item in items
        ]
        if (
            sort_keys is not None
            and serialized_list
            and type(items[0]).__name__ in sort_keys
        ):
