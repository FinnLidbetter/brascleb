...
class TileDistributionView(Resource):
    ...
    def get():
        ...
        player = db.session.query(Player).filter_by(
            user_id=current_user.user_id).options(
            joinedload(Player.distribution).subqueryload(
                Distribution.tile_distribution).joinedload(TileCount.tile)
        ).one()
    ...
    def post():
        ...
        if len(letter_set) != len(data):
            return Response(
                'Duplicate letters included in the data.', status=400)
        player = db.session.query(Player).filter_by(
            user_id=current_user.id).joinedload(Player.distribution).one()
        ...
        for data_tile in data:
            tile = fetch_or_create(
                db.session, Tile,
                letter=data_tile['letter'], value=data_tile['value'],
