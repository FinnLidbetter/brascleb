...
class TokenRefreshView(Resource):
    ...
    def post():
        ...
        data = {"token": access_token, "expiration_date": access_expiration_timestamp}
        return jsonify(data)
    ...
    def post():
        ...
        if device_token is not None:
            device_query = (
                ...
                .filter_by(user_id=user.id, device_token=device_token)
                .one_or_none()
            )
...
class AdminLoginView(Resource):
    ...
    def get():
        form = LoginForm()
        return Response(
            render_template("auth/admin-login.html", title="Sign In", form=form),
            status=200,
        )
...
class RegisterView(Resource):
    ...
    def post():
        ...
        expected_fields = ["username", "password", "confirmed_password", "display_name"]
        ...
        if existing_user:
            ...
            return Response(
                f'Verification email sent to {data["username"]}.', status=200
            )
        new_user = User(
        ...
        default_board_layout = (
            db.session.query(BoardLayout).filter_by(name="Classic").first()
        )
        default_distribution = (
            db.session.query(Distribution).filter_by(name="Classic").first()
        )
        ...
        if current_app.config.get("DEFAULT_OPPONENT_USERNAME") is not None:
            ...
            default_opponent = (
                ...
                .join(Player.user)
                .filter_by(username=default_opponent_username)
                .one()
            )
...
class EmailVerificationView(Resource):
    ...
    def get():
        ...
        if raw_username is not None and raw_token is not None:
            ...
            return Response(
                render_template(
                    "auth/verify_email.html", title="Verify Email", form=form
                ),
                status=200,
            )
    ...
    def post():
        ...
        if not username or not token:
            return Response(
                "Verification failed. Missing username or token.", status=400
            )
        if " " in username:
            return Response(
                "Spaces are not allowed in emails registered to ReRack", status=400
            )
        if not _is_plausible_email(username):
            return Response(
                f"{username} is not a valid email address for use with ReRack",
                status=400,
            )
        ...
        active_records = (
            ...
            .filter(UserVerification.expiration_timestamp > now)
            .all()
        )
...
class RequestVerificationEmailView(Resource):
    ...
    def post():
        ...
        active_records = (
            db.session.query(UserVerification)
            .filter_by(username=username, used=False)
            .filter(now < UserVerification.expiration_timestamp)
            .all()
        )
    ...
    def post():
        ...
        if form.validate_on_submit():
            ...
            verification_records = (
                ...
                .filter(UserVerification.expiration_timestamp > int(time.time()))
                .all()
            )
            verification_record = None
            ...
            db.session.commit()
            return Response(
                f"Success! Password has been reset for {form.username.data}", status=200
            )
        else:
            return Response("Invalid form submission", status=400)
...
class DeviceTokenView(Resource):
    ...
    def post():
        ...
        if device_token is not None:
            device_query = (
                db.session.query(Device)
                .filter_by(user_id=current_user.id, device_token=device_token)
                .one_or_none()
            )
            if device_query is None:
                device = Device(
...
def send_deletion_request_to_admin(requesting_user):
    ...
    message = f"User {requesting_user} has requested deletion of their ReRack account."
    try:
        mail.send_mail(
