...
class GameBoard:
    ...
    def __init__(self, game_query):
        ...
        self.modifiers = [
            [GameBoardModifier(1, 1) for _ in range(self.columns)]
        ...
        for played_tile in game_query.board_state:
            ...
            self.played_tiles[row][column] = GameBoardTile(letter=letter, value=value)
...
class StatefulValidator:
    ...
    def _validate_user_turn(self):
        ...
        if (
            self.game_player is None
            or self.game_player.player.user_id != current_user.id
        ):
    ...
    def _validate_connected(self):
        ...
        for played_tile in self.data:
            ...
            for row_delta, column_delta in adjacent_deltas:
                ...
                if (
                    adj_row < 0
                    or adj_row >= self.game_board.rows
                    or adj_column < 0
                    or adj_column >= self.game_board.columns
                ):
                    continue
...
class WordBuilder:
    ...
    def compute_score(self):
        ...
        if len(self.data) == 1:
            ...
            if (
                self.data[0]["row"] == self.game_board.rows // 2
        ...
        for row_delta, column_delta in delta_pairs:
            ...
            while (
                ...
                if (row, column) in self.data_values:
                    score_sum += (
                        self.data_values[(row, column)]
                        * self.game_board.modifiers[row][column].letter_multiplier
                    )
                    word_multiplier *= self.game_board.modifiers[row][
                        column
                    ].word_multiplier
                elif board_tiles[row][column] is not None:
                ...
                word_len += 1
        if (
            start_row == self.game_board.rows // 2
    ...
    def validate(self):
        ...
        for word in self.words:
            if (
                ...
                .filter(Dictionary.id == self.dictionary_id)
...
class StateUpdater:
    ...
    def update_state(self):
        ...
        tile_count_object_map = fetch_mapped_tile_counts_from_set(
            db.session, tile_counts_set, tile_object_map
        )
        ...
        if not self.game_player.rack and not self.game_state.bag_tiles:
            ...
            combined_score = sum(
                curr_game_player.score
                for curr_game_player in self.game_state.game_players
            )
            for curr_game_player in self.game_state.game_players:
                if (
                    curr_game_player.score
                    > curr_game_player.player.highest_individual_score
                ):
                    curr_game_player.player.highest_individual_score = (
                        curr_game_player.score
                    )
    ...
    def _get_next_bag_and_rack(self):
        ...
        for played_tile in self.data:
            tile_key_letter = (
                played_tile["letter"] if not played_tile["is_blank"] else None
            )
        ...
        num_tiles_to_draw = min(TILES_ON_RACK_MAX - num_tiles_on_rack, num_tiles_in_bag)
        tile_keys = []
        counts = []
        for tile_key, count in next_bag_state.items():
    ...
    def _get_exchanged(self):
        ...
        for exchanged_tile in self.data:
            tile_key = (
                exchanged_tile["letter"],
                exchanged_tile["value"],
    ...
    def _fetch_played_tiles(self, tile_object_map):
        ...
        for datum in self.data:
            ...
            played_tile = fetch_or_create(
                db.session,
                PlayedTile,
                tile_id=tile_object_map[tile_key].id,
                row=row,
                column=column,
            )[0]
