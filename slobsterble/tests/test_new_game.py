...
def test_update_state(db, alice_bob_mutual_friends):
    ...
    created_game = db.session.query(Game).options(
        subqueryload(Game.game_players).subqueryload(
            GamePlayer.rack).joinedload(TileCount.tile),
        subqueryload(Game.bag_tiles).joinedload(TileCount.tile)
    ).order_by(Game.created.desc()).first()
...
def test_valid_gets(client, alice_bob_mutual_friends, alice_carol_friend,
                    alice_headers, bob_headers, carol_headers):
