...
def test_acquire_lock(db):
    ...
    with acquire_lock('test_key'):
        with pytest.raises(AcquireLockException, match='Failed to acquire lock for key: test_key'):
            with acquire_lock('test_key'):
                pass
...
def test_acquire_lock_blocking(db):
    ...
    with acquire_lock('blocking_key', expire_seconds=1):
        initial_expiry = db.session.query(Lock).filter_by(key='blocking_key').one().expiry
        with acquire_lock('blocking_key', block_seconds=3):
            new_expiry = db.session.query(Lock).filter_by(key='blocking_key').one().expiry
...
def test_block_too_short(db):
    ...
    with acquire_lock('blocking_key_2', expire_seconds=60):
        with pytest.raises(AcquireLockException, match='Failed to acquire lock for key: blocking_key_2'):
            with acquire_lock('blocking_key_2', block_seconds=1):
                pass
        # The lock is still in place.
        assert db.session.query(Lock).filter_by(key='blocking_key_2').one_or_none() is not None
