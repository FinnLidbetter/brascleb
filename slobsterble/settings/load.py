...
def load_config(settings, testing=False):
    ...
    settings.SQLALCHEMY_DATABASE_URI = (
        sql_dialect + ":///" + os.path.join(grandparent_path, database_path)
    )
    ...
    settings.APNS_NOTIFICATION_RETRIES_MAX = config.getint(
        "apns", "APNS_NOTIFICATION_RETRIES_MAX"
    )
    ...
    settings.WEB_HOST = config.get(
        "web", "HOST", fallback="slobsterble.finnlidbetter.com"
    )
    ...
    settings.DEFAULT_OPPONENT_USERNAME = config.get(
        "web", "DEFAULT_OPPONENT_USERNAME", fallback=None
    )
