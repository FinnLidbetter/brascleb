...
class NotificationFactory:
    ...
    @staticmethod
    def make_next_turn_notification(
        device_token, game_id, other_player_names, use_sandbox
    ):
        ...
        if len(other_player_names) == 1:
            ...
            other_players_string = (
                f"{other_player_names[0]} and {other_player_names[1]}"
            )
        else:
            other_players_string = (
                f'{other_player_names[:-1].join(", ")}, and {other_player_names[-1]}'
            )
        payload = Payload(
        ...
        notification = Notification(
            payload=payload, token=device_token, use_sandbox=use_sandbox
        )
        return notification

    @staticmethod
    def make_new_game_notification(
        device_token, game_id, creator_name, your_turn, use_sandbox
    ):
        payload = Payload(
