...
Notification = collections.namedtuple(
    "Notification", ["token", "payload", "use_sandbox"]
)
...
class APNsClient:
    ...
    def _connection(self):
        if self._client is None:
            ...
            self._client = httpx.Client(
                base_url=self.base_url, http2=True, limits=self.LIMITS, timeout=10.0
            )
        return self._client
    ...
    def send_notification(
        ...
        headers = self._build_headers(
            payload, topic, priority, expiration, collapse_id, push_type
        )
