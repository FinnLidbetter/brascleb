...
class NewGameLayoutDistributionException(BaseApiException):
    default_message = (
        "The user's board layout is too small for the " "tile distribution."
    )
...
class PlayOverlapException(BaseApiException):
    ...
    default_message = (
        "One or more of the positions of played tiles are " "already occupied."
    )
...
class PlayFirstTurnException(BaseApiException):
    ...
    default_message = (
        "The first played word must go through the centre of " "the board."
    )
