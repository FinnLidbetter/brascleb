...
def init_migrate(app):
    ...
    migrate.init_app(
        ...
        directory=os.path.join(
            os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "alembic"
        ),
        compare_type=True,
    )
...
def init_admin(app):
    ...
    admin.add_view(SlobsterbleModelView(slobsterble.models.BoardLayout, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Device, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Dictionary, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Distribution, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Entry, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Game, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.GamePlayer, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Lock, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Modifier, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Move, db.session))
    admin.add_view(
        SlobsterbleModelView(slobsterble.models.PositionedModifier, db.session)
    )
    admin.add_view(SlobsterbleModelView(slobsterble.models.Tile, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.TileCount, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.PlayedTile, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.Player, db.session))
    admin.add_view(SlobsterbleModelView(slobsterble.models.User, db.session))
    admin.add_view(
...
def init_api(app):
    ...
    api.add_resource(
        DictionaryView, "/api/game/<int:game_id>/verify-word/<string:word>"
    )
